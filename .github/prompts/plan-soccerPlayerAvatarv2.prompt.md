...
4. **Style the UI with CSS** — In `styles.css`, implement:
...
5. **Implement core JavaScript logic** — In `app.js`, write JSDoc-documented functions to:
   - Fetch and parse JSON color data using a loop
   - Populate dropdowns dynamically from JSON
   - `applyColor(itemType, colorName)` — swap the `src` attribute of the corresponding `<img>` element (e.g., `shirt-${colorName}.png`) with conditional validation
